...
class Customer extends Component {
  ...
  render() {
    ...
    const customers = this.state.customers.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trCustomerClick(item)}>
          ...
          <td>
            {item.active === 0 ? (
              /* Gán sự kiện cho nút EMAIL */
              <span className="link" onClick={() => this.lnkEmailClick(item)}>EMAIL</span>
            ) : (
              /* Thêm sự kiện onClick cho nút DEACTIVE */
    ...
    // Render danh sách đơn hàng của khách hàng được chọn
    ...
    // Render chi tiết sản phẩm của đơn hàng được chọn
    if (this.state.order) {
  ...
  // Xử lý khi nhấn nút EMAIL
  lnkEmailClick(item) {
    this.apiGetCustomerSendmail(item._id);
  }

  // Xử lý khi nhấn nút DEACTIVE
  ...
  // API cập nhật trạng thái Deactive
